/**
 * Inspiration advisory -- extracted from Stitch output.
...
export const INSPIRATION_ADVISORY: EmotionAdvisory = {
  ...
  spacing: {
    ...
    rationale:
      'Stitch used gap-10 (40px) between major sections and gap-6 (24px) within sections -- generous throughout. The layout explicitly labels itself "Generous" density with "Asymmetric" flow. Cards use p-6 (24px) internal padding. Unlike Guilt\'s self-imposed compression or Anger\'s crushing density, Inspiration\'s spacing is deliberately expansive. White space is structural, not decorative -- the DESIGN.md calls it "Intentional Air."',
  },
  ...
  motion: {
    ...
    rationale:
      'Stitch created a dedicated .buoyant-bezier CSS class with the exact cubic-bezier(0.22, 1, 0.36, 1) easing. The CREATE button uses active:scale-[1.03] active:-translate-y-[3px] -- a RISING gesture (elements lift on press, opposite of Guilt\'s sinking). The Export button uses hover:scale-[1.03] -- breathing, expanding outward. The hero image uses a 2000ms scale transition on hover for a slow, expansive zoom. Unlike Fear\'s sharp panic (150ms, jarring) or Guilt\'s reluctant sink (300ms, downward), Inspiration rises. Everything moves UP.',
  },
  ...
  depth: {
    ...
    rationale:
      'Stitch created two custom shadow tokens: "buoyant" (0 8px 32px rgba(42,96,144,0.08)) and "buoyant-deep" (0 12px 40px rgba(42,96,144,0.12)). Both use the primary blue tint rather than black -- shadows glow rather than darken. The three elevation tiers (Spark, Lift, Ascend) progress from barely-there to fully buoyant. The DESIGN.md explicitly states: "If the shadow looks like a shadow, it\'s too dark; it should look like a glow of depth." Ghost borders at 20% opacity replace hard lines. Unlike Guilt\'s pressing inset shadows or Anger\'s crushing weight, Inspiration\'s shadows lift elements off the surface.',
  },
  ...
  iconography: {
    ...
    fill: false,
    rationale:
      'Stitch rendered icons in primary (#034877) at text-3xl -- prominent and clear. The 3-column grid with gap-6 gives each icon generous breathing room. Labels are concise and action-oriented: "Idea", "Magic", "Launch", "Sketch" -- each one a creative verb or noun, not an abstract concept. The icon set tells a creation story: idea -> magic -> palette -> draft -> launch -> sketch. Unlike Guilt\'s withdrawn icons (-25 grade, muted) or Anger\'s hostile weight (600+), Inspiration\'s icons are clear, approachable, and forward-looking.',
  },
