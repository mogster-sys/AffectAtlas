...
export const FRIGHT_ADVISORY: EmotionAdvisory = {
  ...
  motion: {
    ...
    rationale:
      'Stitch created a .jolt-active class: active state scales to 0.95 with instant white flash (background #fff, color #000) and transition:none -- the press is INSTANTANEOUS with zero animation. The release uses the 120ms jolt timing. The Tailwind config has transitionDuration "jolt-ms": "120ms" and transitionTimingFunction "jolt": "cubic-bezier(0.8, 0, 1, 0.3)". The SVG bezier visualization shows hard acceleration with a dead stop -- no deceleration, no ease-out. 120ms is the FASTEST duration in the entire system. The "TEST JOLT" button resets text to "SYSTEM RESET..." for 120ms then snaps back. Unlike Anger\'s explosive slam (which hits with power), Fright\'s jolt is about SPEED -- the threat arrives before you can prepare.',
  },
  ...
  shapes: {
    ...
    rationale:
      'Stitch defined a single custom radius "fright": 3px in the Tailwind config. The shapes section shows three options: 0px, 3px (STD, highlighted with primary coloring and italic bold label), and 6px. The note reads: "Sharpness conveys danger. Avoid rounds." Only three radii vs the usual four -- Stitch stripped even the shape options to a minimum. Compare to Fear\'s clinical 6px (precise but not hostile) and Anger\'s absolute 0px (weaponized). Fright\'s 3px sits between: sharp enough to feel dangerous, present enough to feel like a warning sign or emergency panel.',
  },
